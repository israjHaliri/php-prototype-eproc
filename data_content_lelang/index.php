    ...
    <section class="wrapper">
      <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
        ...
        <tbody>
          <?php 
          $no = 1;
          $query = mysqli_query($conn,"SELECT * FROM lelang ORDER BY id_lelang DESC") or die (mysql_error('Tabel tidak ditemukan'));
          while ($data = mysqli_fetch_array($query)){
            ?>

            <tr>
              <td><?php echo $no; ?></td>
              <td><?php echo $data['title_lelang']; ?></td>
              <td><?php echo $data['date_publish_lelang']; ?></td>
              <td align="center"><?php echo '<a href="detail.php?id='.base64_encode($data['id_lelang']).'" class="btn btn-info">Details</a>&nbsp;<a href="edit.php?id='.base64_encode($data['id_lelang']).'" class="btn btn-success">Edit</a>&nbsp;<a href="delete.php?id='.base64_encode($data['id_lelang']).'" class="btn btn-success">Hapus</a>';?></td>
            </tr>
            <?php        
            $no++;
          }
